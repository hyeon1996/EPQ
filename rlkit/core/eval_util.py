...
def get_generic_path_information(paths, stat_prefix='', min_return=None, max_return=None):
    """
    Get an OrderedDict with a bunch of statistic names and values.
    """
    statistics = OrderedDict()
    wandb_statistics = OrderedDict()
    ...
    wandb_statistics.update(create_stats_ordered_dict('Returns', returns, stat_prefix=stat_prefix))
    if min_return is not None and max_return is not None:
        statistics.update(create_stats_ordered_dict('Normalized Returns', (np.array(returns) - min_return)/(max_return - min_return) * 100.0,
                                                    stat_prefix=stat_prefix))
        wandb_statistics.update(create_stats_ordered_dict('Normalized Returns', (np.array(returns) - min_return) / (
                    max_return - min_return) * 100.0, stat_prefix=stat_prefix))
    ...
    return (statistics, wandb_statistics)
