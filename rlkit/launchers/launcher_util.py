...
def init_wandb(exp_prefix, log_dir, variant, project_name=None, **logger_kwargs):
    print(variant['use_wandb'])

    if variant['use_wandb']:
        try:
            import wandb
            wandb.login(key=r'place your wandb key')
            print("Wandb Success")

            wandb_run = wandb.init(project=project_name, config=variant, dir=log_dir, group=logger_kwargs['env_name'], name=exp_prefix, reinit=True)

            seed = (log_dir.split(os.sep)[-1]).split('_')[0]
            # wandb_run.name = exp_prefix + '_' + seed
            wandb_run.name = wandb_run.name + '_' + seed
...
def create_exp_name(exp_prefix, env_name, exp_id=0, seed=0):
    """
...
def create_log_dir(
    ...
    :return:
    """
    exp_name = create_exp_name(exp_prefix, env_name, exp_id=exp_id, seed=seed)
...
def setup_logger(
    ...
    print("Init Wandb")
    init_wandb(exp_prefix, log_dir, variant, **create_log_dir_kwargs)
    ...
    print('rlkit.launchers.launcher_util.py | setup_logger | snapshot_mode: ', logger.get_snapshot_mode())
    print('rlkit.launchers.launcher_util.py | setup_logger | snapshot_gap: ', logger.get_snapshot_gap())
    return log_dir
